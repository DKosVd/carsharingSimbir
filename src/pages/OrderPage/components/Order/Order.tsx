    ...
    <div className={s.order__main}>
      ...
      <div className={s.order__details}>
        {cityPlace?.city.name && (
          <div className={s.order__detail}>
            ...
            <span className={s.order__detail__value}>
              {cityPlace?.city?.name},{" "}
              {cityPlace.address
                ? `${cityPlace?.address?.name}, ${cityPlace?.address?.address}`
                    .split(",")
                    .splice(1)
                    .join(",")
                : ""}
        ...
        {dlc?.color && (
          <div className={s.order__detail}>
            ...
            <span className={s.order__detail__value}>
              {toUpperCase(dlc.color)}
            </span>
          </div>
        )}
        {dlc?.returnDate && dlc?.startDate && (
          <div className={s.order__detail}>
      ...
      {auto && (
        <div className={s.order_price}>
          <h4 className={s.order__price__elem}>
            Цена:{" "}
            {price ? (
              <span className={s.order__price__value}>{price} ₽</span>
            ) : (
              <span className={s.order__price__value}>
                от {preparePrice(auto.priceMin)} до{" "}
                {preparePrice(auto.priceMax)} ₽
              </span>
            )}{" "}
          </h4>
        </div>
      ...
      {numberOrder ? (
        <button className={`btn btn-book btn-detail__red btn-full`}>
          Отменить
        </button>
      ) : (
        <button
          className={`btn btn-book ${
            btnOpt.className ? "btn-detail__red" : ""
          } btn-full`}
          disabled={btnOpt.isDisabled}
