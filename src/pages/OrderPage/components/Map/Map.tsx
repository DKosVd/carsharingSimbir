...
const Map = ({query, place}: IMapProps) => {
    ...
    return (
        <div id="#map">
        <MapContainer  center={[54.32824, 48.38657]} zoom={14}  className={s.map}>
            <TileLayer
                attribution='&copy; <a href="http://osm.org/copyright">OpenStreetMap</a> contributors'
                url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
            />
            {position && <LocationMarker  position={{lat: position[1], lng: position[0]}}/>}
            {pointByCity?.map( el =>  <LocationMarker key={`${el.name}`} position={{lat: el.point[1], lng: el.point[0]}} name={el.name}/>)}
            {pointByCity && pointByCity.map(el => {
                if(el.name === place) {
