...
import { ChoseColorAction, ChoseDateFrom, ChoseDateTo, ChoseDlcRatesAction, ChoseRateAction, FetchRateAction } from '../../../../store/actions/order/order';
...
import { IDlcRate, IRate } from '../../../../store/reducers/order/contracts/state';
...
const ChoseDls = ({ changeActiveBtn, changePage }: IChoseDlsProps) => {
    ...
    const onChangeHandle = (rate: IRate) => {
        if(rate.rateTypeId.id === '5f622f029d3a610b850fd820' && DLC?.startDate && DLC?.returnDate && diffDays(DLC.returnDate as Date, DLC.startDate as Date) < 7) {
            dispatch(ChoseRateAction(rate))
            changeActiveBtn(true);
            changePage(2);
            return
        }
        if (DLC?.startDate && DLC?.returnDate && Math.abs((+new Date(+DLC.returnDate) - +new Date(+DLC.startDate)))){
            dispatch(ChoseRateAction(rate))
    ...
    const choseDateTo = (date: Date | [Date, Date] | null) => {
        ...
        if(DLC?.startDate && !Math.abs((+new Date(+date) - +new Date(+DLC.startDate)))) {
            changeActiveBtn(true);
            changePage(2);
            return
        }
                    ...
                    <div className={s.order__dls__elems}>
                        ...
                        <div className={s.order__dls__row}>
                            <div className="order__form_elem">
                                <input onChange={handleChoseColor} className="custom-radio" id="any" type="radio" name="color" value="any" defaultChecked />
                            ...
                            {DLC?.rates && DLC.rates.map(rate => {
                                return (
                                    <div key={rate.id} className={`order__form_elem ${s.order__dls_rate}`}>
                                        <input onChange={() => onChangeHandle(rate)} className="custom-radio" checked={DLC?.choseRate?.id === rate.id ? true : false} id={rate.id} type="radio" name="rate" value={rate.id} />
                        ...
                        <div className={`${s.order__dls__row} ${s.order__dls__row_bl}`}>
                            <div  className={`order__form_elem ${s.order__dls_rate}`}>
                                <input onChange={() => handleChoseDlcRates(dlcRates[0])} className="custom-checkbox" checked={DLC?.dlc?.fullOil?.isPresent ? true : false} id={`${dlcRates[0].name}`} type="checkbox" value="true" />
                                <label htmlFor={`${dlcRates[0].name}`}>{dlcRates[0].name}, {dlcRates[0].price}р</label>
                            </div>
                            <div  className={`order__form_elem ${s.order__dls_rate}`}>
                                <input onChange={() => handleChoseDlcRates(dlcRates[1])} className="custom-checkbox" checked={DLC?.dlc?.babySeat?.isPresent ? true : false} id={`${dlcRates[1].name}`} type="checkbox" value="true" />
                                <label htmlFor={`${dlcRates[1].name}`}>{dlcRates[1].name}, {dlcRates[1].price}р</label>
                            </div>
                            <div  className={`order__form_elem ${s.order__dls_rate}`}>
                                <input onChange={() => handleChoseDlcRates(dlcRates[2])} className="custom-checkbox" checked={DLC?.dlc?.rightDrive?.isPresent ? true : false} id={`${dlcRates[2].name}`} type="checkbox" value="true" />
                                <label htmlFor={`${dlcRates[2].name}`}>{dlcRates[2].name}, {dlcRates[2].price}р</label>
                            </div>
                        </div>
                    </div>
                </form>
