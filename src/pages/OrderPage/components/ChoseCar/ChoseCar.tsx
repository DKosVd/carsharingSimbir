...
const ChoseCar = ({ changeActiveBtn, changePage }: IChoseCarProps) => {
  ...
  const activeAuto = useSelector(
    (state: RootState) => state.order.choseCar?.id
  );
  ...
  return (
    <>
      ...
      <div
        className={`${s.order__cars} ${
          autosByType.length <= 6 ? `${s.order__car__h}` : ""
        }`}
      >
        {autosByType &&
          autosByType.map((el, idx: number) => {
            return (
              <div
                key={`${idx}__${el}`}
                onClick={() => setActiveCar(el.id, el)}
                className={`${s.order__car}  ${
                  el.id === auto ? s.order__car_active : ""
                }`}
              >
                <div className={s.order__car_content}>
                  <div className={s.order__car_header}>
                    <span className={s.order__car_namecar}>{el.name}</span>
                    <span className={s.order__car_price}>
                      {preparePrice(el.priceMin)} - {preparePrice(el.priceMax)}{" "}
                      ₽
                    </span>
                  </div>
                  <div className={s.order__car_body}>
                    <div
                      className={s.order__car__body_img}
                      style={{
                        backgroundImage: `url(${isBaseImg(el.thumbnail.path)})`,
                      }}
                    ></div>
                  </div>
                </div>
