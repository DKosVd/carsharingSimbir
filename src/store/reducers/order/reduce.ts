...
const initialState: OrderState = {
  ...
  currentStep: 0,
  btnOpt: [
    { name: "Выбрать модель", isDisabled: true },
    { name: "Дополнительно", isDisabled: true },
    { name: "Итого", isDisabled: true },
    { name: "Заказать", isDisabled: true },
  ],
};
...
export const orderReducer = (state = initialState, action: OrderAction) => {
  switch (action.type) {
    ...
    case OrderActionType.CHOSE_RATE:
      let date = dateFormat(
        state.choseDLC?.returnDate as Date,
        state.choseDLC?.startDate as Date
      );
    ...
    case OrderActionType.CHOSE_DATE_TO:
      ...
      if (state.choseDLC?.choseRate) {
        _ = typeRateAndPrice(
          dateFormat(action.payload as Date, state.choseDLC.startDate as Date),
          state.choseDLC.choseRate.rateTypeId.name,
          state.choseDLC.choseRate.price,
          price_
        );
      }
      return {
...
const typeDlcRates = (
  ...
  dlc: dlc,
  price: number
): { name?: string; isPresent?: boolean; price?: number } => {
  if (name === "Полный бак") {
...
const typeRateAndPrice = (
  date: number[],
  type: string,
  plPrice: number,
  price: number
): { price?: number; returnDate?: boolean } => {
  if (type === "На сутки") {
    let currentPrice =
      price +
      plPrice * date[0] +
      Math.floor(plPrice * (date[1] ? date[1] / 24 : 0));
  ...
  if (type === "Недельный Плюс" && date[0] >= 7) {
    let currentPrice =
      price +
      Math.floor((plPrice * date[0]) / 7) +
      Math.floor(plPrice * (date[1] ? date[1] / (7 * 24) : 0));
    return { price: currentPrice };
  }
  if (type === "Поминутно") {
